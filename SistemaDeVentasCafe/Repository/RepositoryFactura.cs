...
namespace SistemaDeVentasCafe.Repository
{
    public class RepositoryFactura : IRepositoryGeneric<Factura>
    {
        ...
        public async Task<Factura?> ObtenerPorId(int id) => await _context.Facturas.Include(f => f.Lista_De_Productos).Include(f => f.fCliente).FirstOrDefaultAsync(f => f.IdFactura == id);
